...
<div class="bg-white/70 flex flex-col p-3 aspect-square backdrop-blur-md rounded-xl align-middle border-4 h-40">
	<div class="flex">
		<p class="font-semibold flex-1 text-lg text-black/70">Svelte</p>
        <!-- Svelte Logo -->
        <div>
	...
	<input 
        class="text-black/80 flex items-center justify-center rounded-md text-l w-28 border-2 m-auto text-center" 
        id="count" 
        bind:value ={count} 
        type="number" 
    />
    <button 
        on:click={set(count)} 
        class="text-dark-800 font-bold text-l flex items-center justify-center m-1 p-1 border-2 rounded-xl"
    >
        Set
    </button>		
</div>